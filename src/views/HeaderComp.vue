<template>
  ...
  <header style="background: #212121"
    ...
    <ul v-if="getTokenIsAdmin.token" v-bind:key='getTokenIsAdmin.token'
      ...
      <li class="ms-5">
        <div class="input-group">
          <input v-model="search_email" type="search" class="border-inpt form-control rounded" placeholder="Search"
                 aria-label="Search" aria-describedby="search-addon"/>
          <button @click="SEARCH_SHOW_POST({email: this.search_email})" type="button" class="btn pink-btn">search
          </button>
        </div>
      </li>
    </ul>
    ...
    <div class="col-md-4 text-end">
      <router-link v-if="getTokenIsAdmin.token" to='/user/home' class="btn pink-btn me-2">{{ getTokenIsAdmin.name }}
      </router-link>
      <router-link v-if="!getTokenIsAdmin.token" to='/user/login' class="btn btn-outline-light me-2">Login</router-link>
      <router-link v-if="!getTokenIsAdmin.token" to='/user/register' class="btn btn-outline-light me-2">Sign-up
      </router-link>
      <router-link v-if="getTokenIsAdmin.is_admin === '1'" to='/adminpanel' class="btn btn-outline-light me-2">Admin
        Panel
      </router-link>
      <router-link v-if="getTokenIsAdmin.token" @click.prevent="logout" to=# class="btn btn-outline-secondary me-2">
        Logout
      </router-link>
    </div>
