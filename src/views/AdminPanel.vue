<template>
  ...
  <table class="table">
    ...
    <tr v-for="person in this.getAdmin" v-bind:key="person.id">
      ...
      <td v-else>user</td>
      <td>
        <router-link :to="{ name: 'UsersPosts', params: { user_id: person.id }}" v-if="person.id !== getUserInfo.id" v-on:click="ADMIN_SHOW_POST({user_id: person.id})" class="btn btn-success">User's Posts</router-link>
      </td>
      <td >
